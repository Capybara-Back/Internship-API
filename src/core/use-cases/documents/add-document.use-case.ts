...
export default class AddDocumentUseCase
    ...
    async perform(
        ...
        const levelOfConfidentialityByFile = formData.metadata.reduce((metadata: Record<string, number>, metadataFile) => {
            metadata[`${metadataFile.filename}`] = metadataFile.levelOfConfidentiality;
            return metadata;
        }, {});
    ...
    private transformFormDocumentToEntities(documents: DocumentFile[], filesUrlDictionary: Record<string, string>, levelOfConfidentialityByFile: Record<string, number>, internshipId: string): Document[] {
        return documents.map((document) => {
            return new Document({
                name: document.name,
                path: filesUrlDictionary[`${document.name}`],
                levelOfConfidentiality: levelOfConfidentialityByFile[`${document.originalName}`],
                internshipId
            });
        });
    }
